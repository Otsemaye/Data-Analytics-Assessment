...
WITH active_accounts AS (
  ...
  WHERE p.is_deleted = 0 AND p.status_id = 1  -- check that the plan is active
)
...
SELECT 
  ...
  CASE 
    ...
    ELSE 'Other'  
  END AS type,  
  COALESCE(MAX(sa.transaction_date), '1900-01-01') AS last_transaction_date,  -- check for NULLs
